varnames <- c(## misc
              ...
              "b659",#(Y=1, N=2, missing or not smoker=-1) Mother has stopped smoking (pregnancy)
              "n5006",#(Y=1, N=2, missing or not smoker=-1) Mother has now stopped smoking  (8y)
              "r6016",#(Y=1, N=2, missing or not smoker=-1) Respondent has now stopped smoking (11y)
              "t5582",
              ## partner smoking
...
stopifnot(length(setdiff(tolower(varnames), tolower(variable.index$name)))==0)
...
alspac$cotinine.prenatal <- with(alspac, set.cond(cotinine_Preg_Trim1_avg, cotinine_Preg_Trim1_avg<0,NA))

alspac$cadmium.prenatal <- with(alspac, set.cond(Cdadj_preg, Cdadj_preg <0, NA))
...
alspac$cotinine.child.15 <- with(alspac, set.cond(Cotinine_TF3, Cotinine_TF3<0,NA))
alspac$cotinine.child.17 <- with(alspac, set.cond(cotinine_TF4_avg, cotinine_TF4_avg<0,NA))
