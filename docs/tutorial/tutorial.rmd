...
>  `r paste(aries.feature.sets(aries.dir), collapse=", ")`.
...
Mendelson et al. identify `r nrow(bmi)` CpG sites associated with BMI.
```{r}
sites <- rownames(fit$coefficient)
common <- intersect(bmi$cpg, sites)
```
Of these, `r length(common)` CpG sites are measured in ARIES. 
...
between the top `r length(common)` CpG site associations
...
kable(tab)
```
(Fisher's exact test p = `r fisher.test(tab, alternative="greater")$p.value`)
